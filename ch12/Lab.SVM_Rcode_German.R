...
fit = svm(y ~., data = german, kernel="radial", decision.values=TRUE)
summary(fit)


## Predicting

fit.pred = predict(fit, newdata = german,  decision.values=TRUE)
...
fit = svm(y ~., data = german.train, kernel="radial", decision.values=TRUE)
summary(fit)
...
fit.pred = predict(fit, newdata = german.train,  decision.values=TRUE)
pred = attributes(fit.pred)$decision.values
...
for(i in 1:V) {
  ...
  fit = svm(y ~., data = german.train, kernel="linear")
