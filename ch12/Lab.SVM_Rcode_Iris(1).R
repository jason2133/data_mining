...
fit = svm(Species ~., data = iris, kernel="linear",  cost=1, decision.values=TRUE, probability=TRUE)
summary(fit)


## Predicting

fit.yhat = predict(fit, newdata = iris, probability=TRUE) 
...
fit = svm(Species ~., data = iris.train, kernel="linear",  cost=1, decision.values=TRUE, probability=TRUE)
summary(fit)


## Predicting

fit.yhat = predict(fit, newdata = iris.test, probability=TRUE)
...
for(i in 1:V) {
  ...
  yhat.test = predict(fit, newdata = iris.test, probability=TRUE)
